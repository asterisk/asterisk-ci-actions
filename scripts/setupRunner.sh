...
if ${INSTALL_PACKAGES} ; then
	...
	# If we're using cached packages, postgres and ODBC need to be
	# reinstalled in order for them to install the ODBC driver
	# configs correctly. 
	apt reinstall postgresql libpq-dev odbc-postgresql unixodbc unixodbc-dev
fi
...
if ${INSTALL_SIPP} ; then
	...
	run_silent_unless_error wget --no-verbose https://github.com/SIPp/sipp/releases/download/${SIPP_VERSION}/sipp-${SIPP_VERSION/v/}.tar.gz
